# Kali.me

<p align="center">
  <img width="300" height="300" src="https://www.kali.org/tools/kali-tweaks/images/kali-tweaks-logo.svg">
</p>

 <h1 align="center">«Ломаем миф — Kali4Kids»</h1>

> **"Установить Kali — не значит стать хакером, так же как покупка скальпеля не делает вас хирургом. Это среда, где мастерство определяется знаниями, а не умением нажимать кнопки в терминале."**

Многие считают Kali Linux либо «игрушкой для детей», либо забагованной системой, которую можно держать только на виртуалке.

### 📚 Обязательно к прочтению
* 📖 **[Official Kali Linux FAQ](https://www.kali.org/faq/)**
* 📘 **[Official Kali Linux Documentation](https://www.kali.org/docs/)**

### 📂 Категории:
1. 🛡️ **[System Hardening](/System%20Hardening/README.md)**
2. 👁️ **[Privacy & Security](./02-privacy)**
3. 🎬 **[Apps & Workstation](./03-apps)**
4. 🔧 **[Troubleshooting](./04-troubleshooting)**
5. 🎨 **[Kali Ricing](./05-kali-ricing)**
